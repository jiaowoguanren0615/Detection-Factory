...
class iFormerBackBone(BaseBackbone):
    ...
    def __new__(
        ...
        **kwargs,
        ...
        # model_config = self.get_instantiate_config(self, iFormer, arch, kwargs)
        # default_weight = model_config.pop("url", None)
        iformer = eval(arch + '()')
        channels = iformer.dims
        default_weight = None
        ...
        self.load_state_dict(iformer, weights)

        # freeze stages
        # self._freeze_stages(self, iFormer, freeze_indices)


        # create feature extractor
        return_layers = [f"stages.{idx}" for idx in return_indices]
        iformer = create_feature_extractor(iformer, return_layers)
        iformer.num_channels = channels
        return iformer

    def _freeze_stages(self, model: nn.Module, freeze_indices: Tuple[int]):
        # freeze stem
        if len(freeze_indices) > 0:
            self.freeze_module(model.stages[0])

        for idx in freeze_indices:
            # freeze layers
            self.freeze_module(model.stages[idx])
            # freeze downsample layers
            if 2 * idx + 2 < len(model.stages):
                self.freeze_module(model.stages[idx])


# if __name__ == '__main__':
#     img = torch.randn(1, 3, 384, 384)
#     model = iFormerBackBone('iFormer_s')
#     y = model(img)
#     for key in y.keys():
#         print(key, y[key].size())
#
#     """
#     stages.0 torch.Size([1, 32, 96, 96])
#     stages.1 torch.Size([1, 64, 48, 48])
#     stages.2 torch.Size([1, 176, 24, 24])
#     stages.3 torch.Size([1, 320, 12, 12])
#     """