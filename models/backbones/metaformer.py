...
class MetaFormerBackbone(BaseBackbone):
    # yapf: enable
    def __new__(
            self,
            arch: str,
        ...
        metaformer = eval(arch + '()')
        default_weight = default_cfgs[arch]['url']

        self.channels = metaformer.channels
        ...
        self.load_state_dict(metaformer, weights)
        ...
        metaformer = create_feature_extractor(metaformer, return_layers)
        metaformer.num_channels = self.channels

        backbone = nn.Sequential(metaformer, PostProcess())
        backbone.num_channels = metaformer.num_channels
...
#     model = MetaFormerBackbone('caformer_m36')
