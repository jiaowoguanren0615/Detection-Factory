...
from models.bricks.base import TwostageTransformer
...
class SalienceTransformer(TwostageTransformer):
    ...
    def forward(
        ...
        # print(f'spatial_shapes: {spatial_shapes.shape[0]}')

        self.alpha = nn.Parameter(torch.Tensor(spatial_shapes.shape[0]), requires_grad=True)
        ...
        # print(f"reverse_multi_level_masks length: {len(reverse_multi_level_masks)}")
        # print(f"valid_token_nums shape: {valid_token_nums.shape}")
        # print(f"self.level_filter_ratio shape: {self.level_filter_ratio.shape}")
